...
<div class="actions">
  <ul>
    <li><?php echo $html->link('Nowa strona', array('action' => 'add')); ?></li>
  </ul>
</div>
...
foreach ($fields as $field)
  ...
  echo '<tr '.$class.'>'; ?>
    <td><?php echo $field['Page']['id']; ?></td>
    <td class="left"><?php echo $field['Page']['title']; ?></td>
    <td class="nowrap"><?php echo $field['Page']['created']; ?></td>
    <td>
      <?php
      echo $html->link('Edytuj', array('action' => 'edit', $field['Page']['id']));
      echo '&nbsp;';
      echo $html->link('Usuń', array('action' => 'delete', $field['Page']['id']), null, 'Na pewno usunąć "'.$field['Page']['title'].'"?');
      ?>
    </td>
  </tr>
<?php
